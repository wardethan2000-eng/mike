...
- Those edits are written straight into the copy, not offered as tracked changes. The user asked for a new document, so there is nothing for them to accept or reject: the old client's wording was never theirs. Leave track_changes out and edit_document does this for you on a fresh copy; only set track_changes to true if the user actually wants to review the change.
- Rewrite by the clause, not by the word. Replace a whole clause or paragraph in a single edit, so a contract takes a handful of edits rather than dozens. Say what the finished document now says; do not describe your own edits as proposed, suggested or pending.
